...
class PortfolioOptimizer:
    ...
    def load_asset_data(self, data_path="Data/"):
        ...
        # Iterate over universe and download data per ticker
        for sector, tickers in self.trade_universe.items():
            for ticker in tickers:
                try:
                    # Download 5 years of daily data; auto_adjust=True gives split/div-adjusted prices
                    ...
                    # Compute daily percentage change to mirror 'Daily_Pct_Change' from CSVs
                    returns = df['Close'].pct_change().dropna()

                    if returns.empty:
                        continue

                    # Keep alignment consistent (index is datetime)
                    returns.name = ticker
                    ...
                    # Silently skip tickers that fail to download to preserve output behavior
                    continue
        ...
        if all_returns:  # PS: Need to perform sanity check to ensure all assets were loaded and their rreturns were calculated.
            ...
            # Each Series in all_returns has .name = ticker, so concat will use those names as columns.
            returns_df = pd.concat(all_returns, axis=1)

            # If something still produced a MultiIndex (e.g., duplicate names), flatten it
            if isinstance(returns_df.columns, pd.MultiIndex):
                returns_df.columns = [c[0] if isinstance(c, tuple) else c for c in returns_df.columns]
...
def main():
    ...
    if optimizer.load_asset_data():
        ...
        if min_var_result['success']:
            ...
            optimizer.display_portfolio_summary(
                max_sharpe_result['weights'],  # (kept as-is to avoid changing outputs)
                "Minimum Variance Portfolio"
            )
